...
$status="0";
$sql = "UPDATE tblblooddonors SET Status=:status WHERE  id=:eid";
...
$sql = "UPDATE tblblooddonors SET Status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
if(isset($_REQUEST['del']))
	{
$did=intval($_GET['del']);
$sql = "delete from tblblooddonors WHERE  id=:did";
$query = $dbh->prepare($sql);
...
<?php $sql = "SELECT * from  tblblooddonors ";
$query = $dbh -> prepare($sql);
$query->execute();
